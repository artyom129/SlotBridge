...
def test_current_client_can_edit_only_profile_fields(client, session):
    ...
    response = client.patch('/auth/me', headers=auth_headers(client, domain.client_a), json={'first_name':'Новый','last_name':'Клиент','phone':'+7 700 000 00 00','role':'ADMIN'})
...
def test_public_version_manifest_has_expected_shape(client):
    ...
    assert response.status_code == 200
    assert response.json()['version_code'] == 4
